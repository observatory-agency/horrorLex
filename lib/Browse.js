class Browse {
  ...
  async categories() {
    ...
    return results.map(({ _id }) => ({
      title: _id,
      uriEncoded: { title: encodeURIComponent(_id) },
    }));
  }

  async films() {
    const results = await this.model.aggregate([{
      $match: { title: { $regex: new RegExp(this.regexChar) } },
    }, {
      $group: { _id: { title: '$title' } },
    }, {
      $sort: { _id: 1 },
    }]).toArray();
    return results.map(({ _id: { title } }) => ({
      title,
      uriEncoded: { title: encodeURIComponent(title) },
    }));
  }
}
